    ...
    try:
        ...
        for out_path, kind, mode in results:
            if kind in ("text", "vocab"):
                ...
                suffix = ".srt" if kind == "srt" else ".mp4"
                u = publish_file(out_path, suffix)
                fname = "subtitle.srt" if kind == "srt" else "subtitled.mp4"
...
async def api_send_chat(request: web.Request) -> web.Response:
    ...
    for f in files:
        ...
        if not path or not os.path.isfile(path):
            continue
        doc = FSInputFile(path, filename=f.get("name"))
        try:
            if str(f.get("name", "")).lower().endswith(".mp4"):
                await BOT.send_video(user.telegram_id, doc,
                                     caption="🎬 Subtitr tayyor!")
            else:
                await BOT.send_document(user.telegram_id, doc)
            sent += 1
        except Exception:
