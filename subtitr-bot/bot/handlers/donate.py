...
async def cmd_donors(message: Message) -> None:
    """Homiylar ro'yxati — minnatdorchilik devori (Mini App bilan bir xil manba)."""
    donors = await list_wall_donations(limit=15)
    if not donors:
        await message.answer(
            "❤️ Hali homiylar yo'q — birinchisi siz bo'ling!\n👉 /donate",
        )
        return
    ...
    await message.answer(
        "🏆 <b>Homiylar — minnatdorchilik devori</b>\n\n"
        + "\n".join(lines)
        + f"\n\n💛 Jami qo'llab-quvvatlash: <b>{total_str} so'm</b>\n"
        "Siz ham qo'shiling 👉 /donate",
        parse_mode="HTML",
    )
