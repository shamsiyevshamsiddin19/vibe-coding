...
(function () {
  ...
  function topicRow(t, lang, idx, isNext) {
    ...
    var editBtn = isReadOnly ? '' :
      '<button class="icon-btn ghost" style="width:34px;height:34px;flex-shrink:0" data-act="topicManage" data-arg=\'' +
      App.arg({ id: t.id, name: t.name, folder: t.folder || '', lang: lang }) + '\'><span data-icon="edit" data-icon-size="15"></span></button>';
  ...
  App.view('grammar', {
    ...
    render: function (page, params) {
      ...
      var plusBtn = isReadOnly ? '' : ('<button class="icon-btn ghost" data-act="topicAdd" data-arg=\'' +
  ...
  function renderMatnTab(t, page, topic, lang, id, nTests) {
    ...
    var isReadOnly = window.Auth && Auth.isReadOnly && Auth.isReadOnly();
    if (menuBtn) {
      if (isReadOnly && !nTests && !topic.game_content) {
        ...
        menuBtn.onclick = function () {
          ...
          if (topic.game_content || !isReadOnly) {
            html +=
