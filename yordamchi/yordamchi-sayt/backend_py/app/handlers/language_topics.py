...
from ..owner import content_read_context, owner_context
...
LIBRARY_LANGS = {
    ...
    "ru_reading", "ru_listening", "ru_speaking", "ru_writing", "ru_shadowing",
}
...
def get_topics(request: Request, body: dict, lang: str):
    ...
    # User'lar admin qo'shgan kontentni ko'radi (o'qish uchun)
    ctx = content_read_context(request)
    rows = db.fetch_all(
...
def get_topic(request: Request, body: dict, topic_id: int):
    # Kontentni admin kontekstidan o'qish (user admin darsliklarini ko'radi)
    ctx = content_read_context(request)
    row = db.fetch_one(
    ...
    # Faollik hisobi foydalanuvchining o'z kontekstida yoziladi
    user_ctx = owner_context(request)
    activity.record(user_ctx, section_for_lang(str(row["lang"])),
