...
def execute_returning(sql: str, params: dict[str, Any] | None = None) -> dict[str, Any] | None:
    """Yozuv + `RETURNING`, natijani qator sifatida qaytaradi.

    DIQQAT: bunga `fetch_one` YARAMAYDI. U `.connect()` ishlatadi va
    tranzaksiyani COMMIT QILMAYDI — ya'ni `INSERT ... RETURNING` yozuvi
    jimgina orqaga qaytariladi. Bir marta shu sabab `idempotency.begin()`
    butunlay ishlamay turgan edi (har chaqiruvda qaytadan insert bo'lardi).
    """
    with get_engine().begin() as conn:
        row = conn.execute(text(sql), params or {}).mappings().first()
        return dict(row) if row else None
...
def init_schema() -> None:
    ...
    with get_engine().begin() as conn:
        # Gunicorn bir nechta worker bilan ishga tushadi va HAMMASI shu DDL'ni
        # bir vaqtda bajarardi — natijada har restartda `DeadlockDetected`
        # chiqardi (app.log da 23 marta). Advisory lock ularni navbatga
        # qo'yadi: birinchisi yaratadi, qolganlari kutib turib `IF NOT EXISTS`
        # tufayli bekorga o'tadi. Lock tranzaksiya tugashi bilan o'zi bo'shaydi.
